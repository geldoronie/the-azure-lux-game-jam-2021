...
{
    [SerializeField] private float _xSpeed = 20f;
    ...
    private void LateUpdate()
    {
        ...
        {
            _currentRotation.x += Input.GetAxis("Mouse X") * _xSpeed * _currentDistance * Time.deltaTime;
            _currentRotation.y -= Input.GetAxis("Mouse Y") * _ySpeed * Time.deltaTime;
        }

        Quaternion rotation = Quaternion.Euler(ClampAngle(_currentRotation.y, _yMinLimit, _yMaxLimit), _currentRotation.x, 0);

        _currentDistance = Mathf.Clamp(_currentDistance - Input.GetAxis("Mouse ScrollWheel") * 5, _distanceMin, _distanceMax);
